...
import org.sagebionetworks.markdown.parsers.MarkdownExtractor;
		...
		for (Iterator iterator = images.iterator(); iterator.hasNext();) {
			...
			if (src.startsWith(WidgetConstants.ENTITY_DESCRIPTION_ATTACHMENT_PREFIX)){
		    	...
		    	if (tokens.length > 5) {
				    ...
				    img.attr("src", ServerMarkdownUtils.createAttachmentUrl(attachmentUrl, entityId, tokenId, tokenId,WidgetConstants.ENTITY_PARAM_KEY));
		    	}
			}
		}
	}
